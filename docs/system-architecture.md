...
- `src/uuid-manager.js`
  Reads Claude config with BOM-tolerant JSON parsing, validates UUID/config shapes and backup payloads, rejects pre-existing temp write paths, creates the first UUID backup, applies a new UUID, and restores the backup.
...
6. If the user applies the roll, UUID manager updates `~/.claude/.config.json`.
...
- Missing Claude config throws a clear setup error.
...
- `CB_ZOO_DATA_DIR` cannot resolve to `.claude` or any nested path inside it.
