...
- UUID management edits only `oauthAccount.accountUuid`, rejects malformed UUIDs, fails closed on invalid config or backup payload shapes, and refuses pre-existing temp paths during writes.
...
- `CB_ZOO_DATA_DIR` is validated so cb-zoo state cannot be redirected into `.claude`.
